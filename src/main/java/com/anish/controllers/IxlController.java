...
public class IxlController {
	...
	public List<Question> getAllRecordsNew(XLData xlData, String name) {
		...
		List<String> existingIds = getCompletedDataByName(xlData.getQuestionCompletedFileName(), destPath, name);
	...
	public List<Question> getQuestionListByStatusNew(XLData xlData, int stauts) {
		return getCurrentXmlFile(xlData.getQuestionFileName()).stream().filter(x -> {
