...
public class IxlController {
	...
	List<Integer> anishGrade = Arrays.asList("4,5".split(",")).stream().map(x -> Integer.parseInt(x)).collect(Collectors.toList());
	List<Integer> ishantGrade = Arrays.asList("1,2".split(",")).stream().map(x -> Integer.parseInt(x)).collect(Collectors.toList());
    ...
    public ModelAndView viewTests(@PathVariable("name") String name) {	
		...
		List<Question> finalDataList = dataList.stream().filter(x -> {
			if(name.equalsIgnoreCase("anish")) {
				...
				return false;
			} else if(name.equalsIgnoreCase("ishant")) {
