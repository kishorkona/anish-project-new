...
public class VocabularyController {
	...
	@GetMapping(value = "/vocabulory/takeTest/{name}/{grade}")
    public ModelAndView getTakeTest(@PathVariable("name") String name, @PathVariable("grade") Integer grade) {
		ModelAndView modelAndView = new ModelAndView();
		try {
			Week week = getAllVocabularyRecords(name, false, 0, 0, grade);
			if(week != null) {
	...
	@GetMapping(value = "/vocabulory/nextTest/{name}/{weekId}/{weekNo}/{grade}")
    public ModelAndView getTakeNextTest(@PathVariable("name") String name, 
    		@PathVariable("weekId") String weekId, @PathVariable("weekNo") String weekNo, @PathVariable("grade") Integer grade) {
		ModelAndView modelAndView = new ModelAndView();
		try {
			Week week = getAllVocabularyRecords(name, true, Integer.valueOf(weekId), Integer.valueOf(weekNo), grade);
			if(week != null) {
	    		modelAndView.addObject("data", week);
	    	}
			modelAndView.setStatus(HttpStatus.OK);
	...
	*/
	...
	public Week getAllVocabularyRecords(String name, boolean next, Integer weekId, Integer workNo, Integer gradeNo) {
		String path = env.getProperty("destination.path");
		String doneFilePath = path+name+"_vocabulary_done.txt";
		List<Week> currentList = getVocabularyFile(name).stream().filter(x -> {
			if(x.getGradeNo()==gradeNo) {
				return true;
			}
			return false;
		...
		if(next) {
			Optional<Week> weekOpt = currentList.stream().filter(x -> {
				if(x.getWeekNo()==weekId && x.getWordNo()==workNo) {
					return true;
				}
				return false;
			}).findFirst();
		...
		Set<String> doneSet = doneList.stream().map(x -> {
			return x.getWeekNo()+"#"+x.getWordNo();
		}).collect(Collectors.toSet());
		List<Week> finalCurrentList = currentList.stream()
				.filter(x -> {
					String key = x.getWeekNo()+"#"+x.getWordNo();
	...
	public void groupData(List<Week> dataList, String name) {
		...
		dataList.stream().forEach(week -> {
			if(weekMap.containsKey(week.getWeekNo())) {
				weekMap.put(week.getWeekNo(), weekMap.get(week.getWeekNo())+1);
			} else {
				weekMap.put(week.getWeekNo(), 1);
			}
			weekSet.add(week.getWeekNo());
		});
		dataList.stream().forEach(week -> {
			week.setTotalWeeks(weekSet.size());
			week.setTotalWords(weekMap.get(week.getWeekNo()));
		});
	}
}
